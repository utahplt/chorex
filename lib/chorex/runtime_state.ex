defmodule Chorex.RuntimeState do
  ...
  def push_inbox(msg, %RuntimeState{} = state), do: %{state | inbox: :queue.cons(msg, state.inbox)}

  @spec drop_inbox(inbox_msg(), t()) :: t()
  def drop_inbox(msg, %RuntimeState{} = state), do: %{state | inbox: :queue.delete(msg, state.inbox)}
