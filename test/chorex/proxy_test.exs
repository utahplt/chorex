defmodule Chorex.ProxyTest do
  ...
  defmodule Worker do
    def test_pingpong() do
      ...
      driver_pid =
        receive do
          {:chorex, _tok, :test, Worker, pid} -> pid
        end
      ...
      m2 =
        receive do
          {:chorex, _tok, :test, Worker, m} -> m
        end

      send(driver_pid, {:got, m2})
    end

    def test_actor_comm(tok) do
      receive do
        {:chorex, ^tok, :meta, {:config, config}} ->
          test_actor_comm_go(config)
      end
    end

    def test_actor_comm_go(config) do
      tok = config.session_token
      send(config[:super], {:chorex, Worker, config})
      send(config[Chorex.ProxyTest.Actor], {:chorex, tok, Worker, Chorex.ProxyTest.Actor, {:from_worker, config[:proxy]}})

      receive do
        {:chorex, ^tok, Chorex.ProxyTest.Actor, Worker, {"hello there", actor_pid}} ->
  ...
  defmodule Actor do
    ...
    def test_actor_comm_go(config) do
      ...
      receive do
        {:chorex, ^tok, Chorex.ProxyTest.Worker, Actor, {:from_worker, the_proxy_pid}} ->
          send(config[:super], {:chorex, Actor, {:got_worker_proxy, the_proxy_pid}})
      end

      send(config[Chorex.ProxyTest.Worker], {:chorex, tok, Actor, Worker, {"hello there", self()}})
    end
  end
  ...
  test "proxy forwards messages" do
    ...
    assert is_pid(proxy)
    tok = UUID.uuid4()
    begin_session(proxy, tok, Worker, :test_pingpong, [])
    send(proxy, {:chorex, tok, :test, Worker, self()})
    assert_receive :worker_here
    i = :rand.uniform(1_000_000)
    send(proxy, {:chorex, tok, :test, Worker, i})
  ...
  test "proxy injects self into config" do
    ...
    config = %{Actor => a1, Worker => proxy, :super => self(), :session_token => tok}
    send(a1, {:chorex, tok, :meta, {:config, config}})
    send(proxy, {:chorex, tok, :meta, {:config, config}})
